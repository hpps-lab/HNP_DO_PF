...
int main(int argc, char** argv)
{
   ...
   const int nend = 360000;
    const int nout =   18000;
    const int nvel =   18000;
...
//<<parameter setting>>//
	...
	const double HHH = 100.0e-6;    //Domain size x [m]
	...
	const double df       = 0.0e+6;  //Reference driving force [J/m^3]
	...
	for(int m1=0; m1<number_of_phase; m1++){
        ...
        if(m0!=m1){
            ...
            MOB_ij[mij] = mobility;
            GAM_ij[mij] = (m0==0 || m1==0) ? gamma : gamma*gAB;
...
//<<Initial profile Setting>>//
	...
	for(int i=0; i<ngx; i++){
		...
		double xx = ((double)i - ncx/4.0)*dx;
		double yy = ((double)j - ncy/2.0)*dy;
		
		if(pi*xx/delta < ps_min){
			PS[0*ngx*ngy + ix] = ps_max;
		}else if(ps_max < pi*xx/delta){
			PS[0*ngx*ngy + ix] = ps_min;
		}else{
			PS[0*ngx*ngy + ix] = -pi*xx/delta;
		}
	
		if(pi*yy/delta < ps_min && ps_max < pi*xx/delta){
			PS[1*ngx*ngy + ix] = ps_max;
		}else if(ps_max < pi*yy/delta || pi*xx/delta < ps_min){
			PS[1*ngx*ngy + ix] = ps_min;
		}else{
			PS[1*ngx*ngy + ix] = -pi*max2(-xx,yy)/delta;
		}
	
		if(ps_max < pi*yy/delta && ps_max < pi*xx/delta){
			PS[2*ngx*ngy + ix] = ps_max;
		}else if(pi*yy/delta < ps_min || pi*xx/delta < ps_min){
			PS[2*ngx*ngy + ix] = ps_min;
		}else{
			PS[2*ngx*ngy + ix] = -pi*max2(-xx,-yy)/delta;
		}
...
//<<<<<<<<<<<<<<<<<<<<<<<<< main loop start >>>>>>>>>>>>>>>>>>>>>>>>>//
	...
	for(int nstep=1; nstep<=nend ; nstep++){
		...
		if(max2(interface_pos_x1_at_y00h,interface_pos_x1_at_y05h) > 0.9*LLL){
