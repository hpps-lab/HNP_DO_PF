...
//<<Initial profile Setting>>//
	...
	for(int i=0; i<ngx; i++){
		...
		}else if(0.5*delta < xx){
			PF[0*ngx*ngy + ix] = pf_min;
		}else{
			PF[0*ngx*ngy + ix] = -sin(pi*xx/delta);
		}
	
		if(yy < -0.5*delta && 0.5*delta < xx){
			PF[1*ngx*ngy + ix] = pf_max;
		}else if(0.5*delta < yy || xx < -0.5*delta){
			PF[1*ngx*ngy + ix] = pf_min;
		}else{
			PF[1*ngx*ngy + ix] = -sin(pi*max2(-xx,yy)/delta);
		}
	
		if(0.5*delta < yy && 0.5*delta < xx){
			PF[2*ngx*ngy + ix] = pf_max;
		}else if(yy < -0.5*delta || xx < -0.5*delta){
			PF[2*ngx*ngy + ix] = pf_min;
		}else{
			PF[2*ngx*ngy + ix] = -sin(pi*max2(-xx,-yy)/delta);
		}
